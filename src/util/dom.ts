...
export function tag(name: string): HTMLElement {
    return document.createElement(name)
} // FIXME: stupid idea with TypeScript
...
export function add(n0: Node, n1: Node): void {
    n0.appendChild(n1)
}
export function remove(n: Node): void {
    if (!n.parentNode) throw Error("element has no parent")
...
export function hasFocus(element: Element): boolean {
    ...
    while (active !== null) {
        ...
        if (active.shadowRoot === null) break
...
export function isNodeBeforeNode(first: Node, second: Node): boolean {
    ...
    for (let child of node!.childNodes) {
        if (child === firstPrevious) return true
        if (child === secondPrevious) return false
    }
...
export function pixelToNumber(pixel: string): number {
    if (pixel === "") return 0
    if (pixel.substr(pixel.length - 2) !== "px") throw Error(`pixelToNumber('${pixel}') expected 'px' suffix`)
