...
export class MenuButton extends ModelView<TextModel> {
    ...
    constructor(master: MenuButtonContainer, node: MenuNode) {
        ...
        if (node.down) {
            if (master.vertical) this.classList.add("menu-side")
            else this.classList.add("menu-down")
        }
        this.updateView()
        ...
        this.onmousedown = (event: MouseEvent) => {
            ...
            let documentMouseUp = function (event: MouseEvent) {
                ...
                setTimeout(() => {
                    if (MenuButton.buttonDown) menuButton.dispatchEvent(new MouseEvent("mouseup", event))
                }, 0)
            }
            document.addEventListener("mouseup", documentMouseUp, { capture: true })
            MenuButton.buttonDown = true

            if (!this.master) throw Error("yikes")
        ...
        this.onmouseover = (event: MouseEvent) => {
            ...
            if (!menuButton.master) throw Error("yikes")
            ...
            switch (menuButton.master.state) {
                ...
                case MenuState.DOWN:
                case MenuState.DOWN_N_INSIDE_AGAIN:
                    if (!MenuButton.buttonDown) break
                    if (!this.master) throw Error("yikes")
                    if (this.master.active) this.master.active.deactivate()
        ...
        if (!this.shadowRoot) throw Error("yikes")
    ...
    override updateView(): void {
        if (this.model && this.model.value) {
            ...
            if (this.model instanceof HtmlModel) span.innerHTML = this.model.value
            else span.innerText = this.model.value

            // FIXME: dom.replaceChild(this.shadowRoot, 1, span)
            if (this.shadowRoot.children.length > 1) this.shadowRoot.removeChild(this.shadowRoot.children[1])
            if (this.shadowRoot.children.length > 1) this.shadowRoot.insertBefore(span, this.shadowRoot.children[1])
            else this.shadowRoot.appendChild(span)
        }
    ...
    isEnabled(): boolean {
        if (this.node.down !== undefined) return true
    ...
    collapse(): void {
        if (!this.master) throw Error("yikes")
        if (this.master.parentButton) this.master.parentButton.collapse()
        else this.deactivate()
    }

    openPopup(): void {
        if (!this.node) return
        if (!this.node.down) return
    ...
    closePopup(): void {
        ...
        if (this.popup.active) this.popup.active.deactivate()
        this.popup.hide()
    }
    ...
    deactivate(): void {
        if (!this.master) throw Error("yikes")
        if (this.master.active !== this) return
